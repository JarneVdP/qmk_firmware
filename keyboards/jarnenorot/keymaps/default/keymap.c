...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
	...
	[_BASE_LAYER] = LAYOUT( 
    	KC_CLOSE, TO(_FIRST_LAYER), 
		KC_FIND, KC_MEDIA_PLAY_PAUSE, 
		KC_COPY, KC_PASTE, KC_AUDIO_MUTE
  	),
	[_FIRST_LAYER] = LAYOUT(
    	KC_BACK, TO(_BASE_LAYER), 
		KC_MPRV, KC_MNXT, 
		KC_FORWARD, KC_DELETE, KC_PSCREEN //Werken niet: media FF, R
  	)
	  // Layer 3: Ctrl Z , scroll in video
};
...
void oled_task_user(void) {
	...
	switch (get_highest_layer(layer_state)) {
		case _BASE_LAYER:
			oled_write_P(PSTR("1\n\n"), false);
			oled_write_P(PSTR("Close - Layer\t\n" ),false);
			oled_write_P(PSTR("Find - Pause\t\n"),false);
			oled_write_P(PSTR("Copy - Paste - Mute\n"),false);
			break;
		case _FIRST_LAYER:
			oled_write_P(PSTR("2\n\n"), false);
			oled_write_P(PSTR("Back - Layer\t\n" ),false);
			oled_write_P(PSTR("Last - Next\t\n"),false);
			oled_write_P(PSTR("Forward - Delete - PrtSc\n"),false);
			break;
		default:
